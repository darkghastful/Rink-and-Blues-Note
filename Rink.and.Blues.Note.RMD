...
```{r}
# setwd("/Hockey")
.libPaths(c("/AppData/Local/R/win-library/4.3", "C:/Program Files/R/R-4.3.1/library"))
package.list <- c("devtools", "ape", "scales", "rlang", "gtools", "ggplot2", "dplyu", "ggpubr", "remotes", "plyu", "egg",
                  "xtable", "kableExtra", "DescTools", "glue", "paletteer", "ggplot2", "magrittr", "ggforce",
                  "ggrepel", "grid", "rstatix", "data.table", "tidyverse", "ggpubr", "ggprism", "patchwork", 
                  "dplyu", "ggtext", "extrafont") #c("lemon", "RColorBrewer", "randomcoloR", "viridis", "mmtable2", "ShortRead", )
...
# My function subset.object is needed to run 

# Establish path to the location of hockey analysis folder
hockey.path <- "/Hockey"

# Direct to and read the .csv file containing the rink specifications
rink.frame <- read.csv(paste0(hockey.path, "/Import/Rink.frame.csv"))

rink.y <- 90
rink.x <- (200/85)*90
rink.scale <- 10
# rink.frame[,"size"] <- ((rink.frame[,"size"]*1.333333) * 25.4)/rink.scale
...
endzone.faceoff.segments <- subset.object(subset.object(rink.frame, "endzone.faceoff", "element"), "segment", "geom")

faceoff.circle <- subset.object(subset.object(rink.frame, "endzone.faceoff", "element"), "circle", "geom")
...
rink.frame <- rbind(subset.object(rink.frame, "endzone.faceoff", "element", remove=TRUE), faceoff.circle, endzone.faceoff.segments)
...
for(b in 1:length(layers)){
  ...
  rink.test <- rink.test +
    geom_circle(data=subset.object(rink.frame.layer, "circle", "geom"), inherit.aes=FALSE, 
                 aes(x0=x, y0=y, r=r, size=size, color=color, fill=fill))
  
  # Add segments to the plot 
  rink.test <- rink.test +
    geom_segment(data=subset.object(rink.frame.layer, "segment", "geom"), inherit.aes=FALSE, 
                 aes(x=x, xend=xend, y=y, yend=yend, size=size, color=color)) 
}
...
blues.logo.frame <- read.csv(paste0(hockey.path, "/Import/Blues.logo.csv"))
...
logo.test <- ggplot() +
  ...
  geom_segment(data=subset.object(blues.logo.frame, "segment", "geom"), aes(x=x, xend=x.end, y=y, yend=y.end), size=size, lineend="round")

blues.logo.frame.curve <- subset.object(blues.logo.frame, "curve", "geom")
...
for(a in 1:length(direction)){
  ...
  # blues.logo.frame[, direction[a]] <- (blues.logo.frame[, direction[a]]*0.75)
  # blues.logo.frame[, paste0(direction[a], ".end")] <- (blues.logo.frame[, paste0(direction[a], ".end")]*0.75)
}
...
# rink.logo.test+ theme(axis.line.x.bottom=element_line(), axis.line.y=element_line())
```